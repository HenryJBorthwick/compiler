...
class If_Else_AST: # ADDED
    def __init__(self, condition, then, else_=None): # default value for else_
        self.condition = condition
        self.then = then
        self.else_ = else_
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + repr(self.then) + \
               (' else ' + repr(self.else_) if self.else_ else '') + ' end'
    def indented(self, level):
        result = indent('If-Else', level) + \
               self.condition.indented(level+1) + \
               self.then.indented(level+1)
        if self.else_ is not None:
            result += self.else_.indented(level+1)
        return result
...
#     p[0] = If_Else_AST(p[2], p[4], p[6] if len(p) > 6 else None)

def p_if_with_else(p): # ADDED
    'If : IF Comparison THEN Statements ELSE Statements END'
    p[0] = If_Else_AST(p[2], p[4], p[6])

def p_if_without_else(p): # ADDED
    'If : IF Comparison THEN Statements END'
    p[0] = If_Else_AST(p[2], p[4])

def p_write(p): # ADDDED
